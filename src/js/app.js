...
const ENDPOINT_EVAL = `${PROTRACK_BASE}/api/public/eval?position_id=${encodeURIComponent(positionId)}`;
...
async function loadPositions() {
  ...
  try {
    ...
    sel.innerHTML = `<option value="">No se pudo cargar</option>`;
    showFormError("No se pudieron cargar cargos. Revisa CORS (PUBLIC_ALLOWED_ORIGINS) o X-API-Key.");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Si cargó bien y hay más de 1 opción (Selecciona + cargos)
  const roleSel = $("role");
  if (roleSel && roleSel.options && roleSel.options.length > 1) {
